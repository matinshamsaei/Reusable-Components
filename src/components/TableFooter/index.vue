...
const to = computed(() => {
  if (isLastPage) return props.total
...
function checkPagination(perPage: number) {
  let to
  let from
  if (typeof props.total === 'number' && perPage * currentPage >= props.total) to = props.total
  else to = perPage * currentPage
    ...
    <div class="row d-flex justify-content-between">
      <div v-if="props.showTotal" class="col-sm col-12 d-flex align-items-center text-dark mb-3 ms-2 my-sm-1">
        <div class="mx-auto mx-sm-0">
          <h6 id="from-to" class="d-inline-block font-md mb-0">{{ from }} {{ $t('shared.until') }} {{ to }}</h6>
      ...
      <div v-if="props.showPerPage" class="d-flex me-1 my-2 col-sm col-12">
        <div class="d-flex align-items-center font-md mx-auto me-md-0 ms-md-auto">
          ...
          <RFormSelect v-model="size" size="sm" class="text-dark transparent-input" :options="sizeOptions" />
        </div>
      </div>
    </div>
  </div>
